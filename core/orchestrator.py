...
# NEW: Import the search_notes tool
from core.tools import get_current_time, save_memory, find_object, search_notes
...
class AIAssistant:
    ...
    def start_new_chat(self):
        ...
        if self.current_model_index < len(self.available_models):
            ...
            # Optionally add google_search tool to config if you want web fallback
            # tools=[get_current_time, save_memory, find_object, search_notes, {"google_search": {}}]
            
            self.chat = self.client.chats.create(
                model=model_name, 
                config=types.GenerateContentConfig(
                    # Add search_notes to the AI's toolbelt!
                    tools=[get_current_time, save_memory, find_object, search_notes] 
                )
            )
